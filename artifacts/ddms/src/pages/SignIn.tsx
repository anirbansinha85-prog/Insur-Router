...
export default function SignIn() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    login.mutate(
      ...
      {
        onSuccess: () => {
          // Reload rather than patch the cache.
          //
          // Cache surgery here was subtly wrong: clearing removes the /auth/me
          // query the gate is watching, and seeding it afterwards left the
          // observer holding the error from the pre-login 401 — the cookie was
          // set, the server was happy, and the screen still showed the form.
          //
          // A reload is also the stronger guarantee. Identity has changed, and
          // nothing fetched as somebody else should survive that. Belt and
          // braces for a once-per-session event.
          queryClient.clear()
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 px-4">
      <div className="w-full max-w-sm">
        <div className="flex items-center gap-2.5 mb-6">
          ...
          <div>
            <div className="font-bold text-slate-900 leading-tight">DDMS</div>
        ...
        >
          <div>
            ...
            <input
              ...
              className="w-full h-10 rounded-md border border-slate-300 px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
            />
          </div>
        ...
        <p className="text-[11px] text-slate-400 mt-4 text-center">
          Each sign-in sees only its own group's showrooms.
        </p>
      </div>
    </div>
  )
