    ...
    <div className="px-4 py-3 flex items-start gap-4">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="w-28 text-right">
          {s.isDefault ? (
            <span className="text-[11px] text-slate-400">ours: {s.unit === "rank" ? RANK_WORD[s.default] : s.default}</span>
          ) : (
            <button
              onClick={() => save(null)}
              disabled={!editable || set.isPending}
              className="text-[11px] text-slate-500 hover:text-slate-900 inline-flex items-center gap-1"
              title={`Back to ${s.unit === "rank" ? RANK_WORD[s.default] : s.default}`}
            >
              <RotateCcw className="w-3 h-3" />
              back to {s.unit === "rank" ? RANK_WORD[s.default]?.toLowerCase() : s.default}
            </button>
          )}
        </div>
      </div>
